...
namespace Shutdown
{
    public class Firewall:IShutdownable
    {
        ...
        {
            Logs.Firewall.Info($"Opening ports [{ string.Join(',', ports)}]");
            lock (_LockObjectDispose)
            {
                if (_Disposed) return;
            }
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) return;
            Logs.Firewall.Info("Was linux so proceeding to open ports");
        ...
        public void Dispose()
        {
            ...
            Logs.Firewall.Info($"Closing ports {string.Join(',', _PortsOpened)}");
            lock (_PortsOpened) {
